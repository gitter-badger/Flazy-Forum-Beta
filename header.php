...
if ($forum_user['pm_new'] && $forum_config['o_pm_show_new_count'] || $pm_full) {
	...
	if (!$forum_config['o_pm_inbox_size']) {
		$query = array('UPDATE' => 'pm', 'SET' => 'status=\'delivered\'', 'WHERE' => 'receiver_id=' . $forum_user['id'] . ' AND status=\'sent\'', );
		...
		$query = array('SELECT' => 'm.id', 'FROM' => 'pm AS m', 'WHERE' => 'm.receiver_id=' . $forum_user['id'] . ' AND m.status=\'sent\'', 'ORDER BY' => 'm.edited', 'LIMIT' => ($forum_config['o_pm_inbox_size'] - $forum_user['pm_inbox']));
		...
		if ($forum_db -> num_rows($result)) {
			...
			$query = array('UPDATE' => 'pm', 'SET' => 'status=\'delivered\'', 'WHERE' => 'id IN (' . substr($ids, 0, -2) . ')', );
...
if ($forum_user['g_id'] == FORUM_ADMIN) {
	...
	if ($forum_config['o_check_for_updates']) {
		if ($forum_updates['fail'] || isset($forum_updates['version']) && isset($forum_updates['hotfix']) || isset($forum_updates['version']) || isset($forum_updates['hotfix']))
			$alert_items = true;
	}

	if ($forum_config['o_database_revision'] > FORUM_DB_REVISION || ($db_type == 'mysql_innodb' || $db_type == 'mysqli_innodb') && $forum_config['o_database_engine'] != 'InnoDB' || ($db_type == 'mysql' || $db_type == 'mysqli') && $forum_config['o_database_engine'] != 'MyISAM')
		$alert_items = true;
