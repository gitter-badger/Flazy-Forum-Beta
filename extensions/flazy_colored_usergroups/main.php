<?php
/**
 * flazy_colored_usergroups functions file
 *
...
function cache_flazy_coloured_usergroups()
{
	global $forum_db;
	...
	if (!empty($output))
	{ 
		$fh = @fopen(FORUM_CACHE_DIR.'cache_flazy_coloured_usergroups.php', 'wb');
		if (!$fh)
		...
		fwrite($fh, '<?php'."\n\n".'$flazy_colored_usergroups_cache = \''.implode(" ",$output)."';\n".'?>');
