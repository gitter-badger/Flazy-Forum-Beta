<?php
/**
 * Български езиков пакет.
 * @package Flazy_Bulgarian
 */

/** Езиковият пакет се използва в  viewforum.php */
$lang_forum = array(

'Forum subtitle'			=>	'%1$s в този форум с подробности за %2$s.',
'Search subtitle'			=>	'%1$s намерини с подробности за %2$s.',
'Topics'					=>	'Теми',
'Replies'					=>	'Отговори',
'Forum'						=>	'Форум',
'Reply'						=>	'Отговор',
'Views'						=>	'Преглеждания',
'Last post'					=>	'Последно мнение',
'Select topic'				=>	'Избери тема: %s.',
'by poster'					=>	' от %s',
'Item status'				=>	'%s:',
'Topic starter'				=>	'Автор: %s',
'New posts'					=>	'Нови мнения',
'Topic navigation'			=>	'( %s )',
'Location'					=>	'Намерено във форум: %s',
'Pages'						=>	'Страници',
'Users online'				=>	'Форума се разглежда от:',
'Post topic'				=>	'Публикувай нова тема',
'Login to post'				=>	'Трябва да %1$s или %2$s за да публикувате нова тема',
'No permission'				=>	'Не е разрешено публикуването на нови теми',
'Moved'						=>	'Преместено:',
'Sticky'					=>	'Важна',
'Closed'					=>	'Заключена',
'Poll'						=>	'Анкета',
'Description'				=>	'Описание',
'Empty forum'				=>	'Празен форум',
'No topics'					=>	'Няма публикувани теми',
'First topic'				=>	'Вие ще бъдете първите, които я <a href="%s">създават</a>',
'You posted indicator'		=>	'·',
'Permalink forum'			=>	'Перманентен линк към този форум.',
'Forum options'				=>	'Форум опции',
'Moderate forum'			=>	'Модерирай форум',
'RSS forum feed'			=>	'RSS теми във форума',
'RSS forum posts feed'      =>  'RSS форум мнения',
'Mark forum read'			=>	'Отбележи форума като прочетен',
'New posts info'			=>	'Отидете на първото публикувано мнение след последното ви влизане.'

);